...
const About = ({ pageContext, data }) => {
    ...
    return (
        <Layout pageContext={pageContext}>
            <Seo title={`Nosotros | ${data.site.siteMetadata.title}`} lang={pageContext.langKey} description={data.markdown.frontmatter.subtitle} />
            {/* header */}
            <div style={{minHeight: '40vh', backgroundColor: 'rgba(0,0,0,0.45)', alignItems: 'center'}} className='bgImgContainer overlay'>
            ...
            <Section>
                <Grid container>
                    <Grid item sm={4} style={{display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center'}}>
                        <h2 style={{wordWrap: 'break-word', marginLeft: 15, textTransform: 'uppercase'}}>NOS ENFOCAMOS EN PRODUCIR GRANDES VACAS CON EXCELENTE MORFOLOGÍA, LONGEVAS Y QUE PRODUZCAN GRANDES CANTIDADES DE LECHE CON EXCELENTES COMPONENTES</h2>
                        <h2 style={{wordWrap: 'break-word', marginLeft: 15, textTransform: 'uppercase'}}> Estamos PREOCUPADOS POR EL MEJORAMIENTO GENÉTICO DE LA RAZA HOLSTEIN DESDE HACE {new Date().getFullYear() - (new Date("01/12/1961").getFullYear())} AÑOS</h2>
                    </Grid>
                    <Grid item sm={8}>
...
export const query = graphql`
    {
        data: allContentfulVaca(
            filter: {node_locale: {eq: "es-MX"}, isHistoric: {eq: true}}
        ...
        markdown: markdownRemark(frontmatter: {lang: {eq: "es"}, templateKey: {eq: "about"}}) {
            frontmatter {
                title
                subtitle
                section_2 {
